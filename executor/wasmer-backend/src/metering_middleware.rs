...
/// to non-linear costs of a gas metering overhead on real world code. Fixed scaling factor is used
...
fn cost_function(operator: &Operator) -> u64 {
    let cycles = match operator {
        // Validated by benchmarks,
        // tweaked manually where appropriate to adjust for possible noise in the benchmarks.
        Operator::I32Const { .. } => 1,
        Operator::I64Const { .. } => 1,
        Operator::F32Const { .. } => 1,
        Operator::F64Const { .. } => 1,
        ...
        // Common operators for which there is either no support (i.e. "exotic" proposals such as SIMD) or no need to meter (i.e. control flow operators).
        Operator::Unreachable => 1, // Called only once as it's causing a trap.
        Operator::Nop => 1, // This is a no-op.

        // These are control flow operators usually marking the beginning or end of a block that may not have observable cost after compiling Wasm into a native code, but we'll still put 1 cycle per each to charge for this.
        Operator::Block { .. } |
        Operator::Loop { .. } |
        Operator::Else => 1,
        Operator::TryTable { .. } |
        ...
        // These are also control flow operator marking end of beggining of a block that may not translate to a specific x86_64/arm opcode.
        Operator::End |
        Operator::Return |
        Operator::ReturnCall { .. } |
        Operator::ReturnCallIndirect { .. } => 1,

        // Dropping value from the stack should be the same cycle count as pushing value.
        Operator::Drop => 1,

        Operator::TypedSelect { .. } => unreachable!(),

        // Same cycle count as local.get
        Operator::LocalSet { .. } => 1,
        ...
        Operator::BrOnNull { .. } |
        Operator::BrOnNonNull { .. } => todo!("{operator:?}"),
    };

    let cost = cycles * MULTIPLIER / SCALING_FACTOR;
