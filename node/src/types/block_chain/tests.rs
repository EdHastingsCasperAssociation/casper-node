...
use crate::types::block_chain::block_chain_entry::BlockChainEntry;
use crate::types::block_chain::indexed_block_chain::IndexedBlockChain;
use crate::{
    types::block_chain::{BlockChain, Error},
...
fn should_remove() {
    ...
    assert_eq!(false, block_chain.is_empty(), "chain should not be empty");
...
fn should_find_all_actual_entries() {
    ...
    assert_eq!(
        expected.iter().map(|x| x).cloned().collect_vec(),
        all,
...
fn should_find_all_virtual_entries() {
    ...
    assert_ne!(
        all,
        range.iter().map(|x| x).cloned().collect_vec(),
