        ...
        + Send,
{
    ...
    let cancel_message = async move { while close_this.changed().await.is_ok() {} };
    ...
    P: Payload,
{
    loop {
        select! {
            ...
            closed_res = close_this.changed() => {
                if closed_res.is_err() {
